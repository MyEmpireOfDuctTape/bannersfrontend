  ...
  methods: {
    ...
    upload(file) {
      ...
      .then((response) => {
          ...
          this.onChangeAnexo()
          this.uploadedData.fileName = '???'
          this.uploadedData.fileSize = this.returnFileSize(response.data.file.size)
          this.uploadedData.fileType = this.returnFileType(response.data.file.type)
          this.uploadedData.fileUrl = response.data.file.publicUrl
          this.$emit('success', {originalFile: response.data.file, displayData: this.uploadedData, names: this.fileNames})
          this.progress = 0
          this.cleanInput()
      })
      .catch((error) => {
